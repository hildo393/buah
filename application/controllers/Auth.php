...
class Auth extends CI_Controller
{
	...
	function create_pass(){
		echo $this->bcrypt->hash_password('administrator'.' : ');
		echo $this->session->userdata('logedin');
	}

	public function user_log()
	{
		$this->data['title'] = 'Histori Login';
		...
		if($this->session->userdata('logedin') != 'TRUE'){ redirect('auth/login', 'refresh');}
			...
			$this->data['log'] 	= $this->Ion_auth_model->get_all_log()->result();

			$this->_render_page('auth/log', $this->data);
		}
	}
	...
	public function pembeli()
	{
		...
		elseif (!$this->ion_auth->is_superadmin()){
			...
			$this->data['users'] 	= $this->ion_auth->get_all_pembeli()->result();

			$this->_render_page('auth/pembeli', $this->data);
		}
	}
	...
	public function reseller()
	{
		...
		elseif (!$this->ion_auth->is_superadmin()){
			...
			$this->data['users'] 	= $this->ion_auth->get_all_reseller()->result();

			$this->_render_page('auth/reseller', $this->data);
		}
	}
		...
		if ($this->form_validation->run() == FALSE /*|| !isset($response['success']) || $response['success'] <> TRUE*/)
		{
			...
			else
			{
				...
				//checking data via model
					$checking = $this->Ion_auth_model->get_by_username($this->input->post('identity'));
					//jika ditemukan, maka create session
					if ($this->bcrypt->check_password($this->input->post('password'), $checking->password)) 
				{
					$session_data = array(
								'id'           => $checking->id,
								'name'     	=> $checking->name,
					...
					$this->Ion_auth_model->insert_log($data);
	...
	public function edit_user($id){
 	...
 	public function profil()
 	{
 		...
 		$this->load->view('auth/profil', $this->data);
 	}
