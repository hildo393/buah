...
class Testimoni_model extends CI_Model
{
  ...
  var $column = array('id_testimoni','judul_testimoni');
  ...
  function get_all_new_home()
  {
    ...
    $this->db->select('name,photo,photo_type,isi_testimoni');
    $this->db->join('users', 'users.id=testimoni.users_id');
    $this->db->order_by($this->id, $this->order);
    $this->db->limit(10);
    return $this->db->get($this->table)->result();
  }
  ...
  function get_all_testimoni($per_page,$dari)
  {
    $this->db->select('photo,photo_type,isi_testimoni');
    $this->db->join('users', 'users.id=testimoni.users_id');
    $this->db->order_by($this->id, 'DESC');
    $query = $this->db->get($this->table,$per_page,$dari);
    return $query->result();
