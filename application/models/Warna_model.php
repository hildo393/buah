...
class Warna_model extends CI_Model
{
  ...
  var $column = array('id_warna','judul_warna');

  private function _get_datatables_query()
  {
    $this->db->join('kategori', 'warna.kategori_id = kategori.id_kategori', 'left');
  ...
  {
  	$sql_prov=$this->db->get('warna');
  	if($sql_prov->num_rows()>0)
    {
  		foreach ($sql_prov->result_array() as $row)
			{
				$result['']= '- Pilih Warna -';
				$result[$row['id_warna']]= ucwords(strtolower($row['judul_warna']));
			}
			return $result;
		}
	}
  ...
  function get_list_by_warna($slug, $limit=null, $offset=null)
  {
    $this->db->join('warna', 'produk.kat_id=kategori.id_warna');
  ...
  function get_all_warna_sidebar()
  {
    $this->db->order_by('judul_warna', 'asc');
    return $this->db->get($this->table)->result();
  }

  function get_total_row_warna()
  {
